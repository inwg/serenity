...
local Tabs = {
    ...
    Weapons = Window:AddTab({ Name = "Weapons", Icon = "crosshairs" }),
...
local SkinsBox = Section(Tabs.SkinChanger, "Skins", "left")
local Skins2Box = Section(Tabs.SkinChanger, "Skins 2", "right")
...
for i = 1, midPoint do
    ...
    registry[tostring(skin)] = SkinsBox:AddLabel(tostring(skin)):AddDropdown({
        Values = children,
        Default = children[1],
        Multi = false,
    })
...
for i = midPoint + 1, #skinList do
    local skin = skinList[i]
    local children = skin:GetChildren()
    registry[tostring(skin)] = Skins2Box:AddLabel(tostring(skin)):AddDropdown({
        Values = children,
        Default = children[1],
        Multi = false,
    })
...
do
    local WeaponModsBox = Section(Tabs.Weapons, "Weapon Mods", "left")
    local GernadesBox = Section(Tabs.Weapons, "Gernades", "right")

    Toggle(GernadesBox, "Antiflashbang", "Enable No Flashbang", false)
    Toggle(GernadesBox, "Antismoke", "Enable No Smoke", false)
    ...
    local VisualsESPBox = Section(Tabs.Visuals, "ESP", "left")

    Toggle(VisualsESPBox, "ESPEnabled", "ESP Enabled", false)
    Dropdown(VisualsESPBox, "ESPBoxType", "Box ESP", { Values = { "2D Box", "3D Box", "Corner Box", "Disabled" }, Default = "2D Box", Multi = false })
    Toggle(VisualsESPBox, "ESPName", "Name ESP", false)
    ColorPicker(VisualsESPBox, "ESPNameColor", "Name Color", Color3.new(1, 1, 1))
    Toggle(VisualsESPBox, "ESPHealth", "Health Bar", false)
    Toggle(VisualsESPBox, "ESPDistance", "Distance ESP", false)
    Toggle(VisualsESPBox, "ESPTracer", "Tracer ESP", false)
    Dropdown(VisualsESPBox, "ESPTracerOrigin", "Tracer Origin", { Values = { "Bottom", "Top", "Center", "Mouse" }, Default = "Bottom", Multi = false })
    Toggle(VisualsESPBox, "ESPSkeleton", "Skeleton ESP", false)

    local VisualsChamsBox = Section(Tabs.Visuals, "Chams", "right")
    Toggle(VisualsChamsBox, "ESPChams", "Enable Chams", false)
    local lblChamsFill    = ColorPicker(VisualsChamsBox, "ChamsFillColor", "Fill Color", Color3.fromRGB(255, 0, 128))
    local lblChamsOutline = ColorPicker(VisualsChamsBox, "ChamsOutlineColor", "Outline Color", Color3.fromRGB(255, 255, 255))
    Dependency("ESPChams", true, { lblChamsFill, lblChamsOutline })
end
...
getgenv().esplib_get_color = function(character)
    if character and character.Parent and TEAM_COLORS[character.Parent.Name] then
        return TEAM_COLORS[character.Parent.Name]
    end
    local player = Players:GetPlayerFromCharacter(character)
    if player then return getPlayerTeamColor(player) end
    return DEFAULT_ESP_COLOR
end
...
local chamsCfg = {
    enabled = false,
    fill = Color3.fromRGB(255, 0, 128),
    outline = Color3.fromRGB(255, 255, 255),
    fillTransparency = 0.5,
    outlineTransparency = 0,
}
local chamsHighlights = setmetatable({}, { __mode = "k" })

local function removeChams(character)
    local hl = chamsHighlights[character]
    if hl then
        pcall(function() hl:Destroy() end)
        chamsHighlights[character] = nil
    end
end

local function applyChams(character)
    if not character or not chamsCfg.enabled then return end
    ...
    if not hl then
        ...
        hl.Parent = character
        chamsHighlights[character] = hl
    end
    hl.FillColor = chamsCfg.fill
    hl.OutlineColor = chamsCfg.outline
...
local function isLocalModel(model)
    return model and model.Name == LocalPlayer.Name
end

local function attachModel(model)
    if not model or isLocalModel(model) then return end
    if espActive then
        addEspToCharacter(model, Players:FindFirstChild(model.Name))
    end
    applyChams(model)
end

local teamFolderConns = {}
local function watchTeamFolder(folder)
    if not folder then return end
    if not teamFolderConns[folder] then
        teamFolderConns[folder] = true
...
local function refreshAllCharacters()
    watchTeamFolder(get_t())
    watchTeamFolder(get_ct())
end

local function updateChams()
    chamsCfg.enabled = Toggles.ESPChams.Value
    chamsCfg.fill = Options.ChamsFillColor.Value
    chamsCfg.outline = Options.ChamsOutlineColor.Value
    if chamsCfg.enabled then
        refreshAllCharacters()
        for character in next, chamsHighlights do
...
Toggles.ESPChams:OnChanged(function()
    updateChams()
end)
...
if charfolder then
    charfolder.ChildAdded:Connect(function(child)
        if child.Name == "Terrorists" or child.Name == "Counter-Terrorists" then
            watchTeamFolder(child)
        end
    end)
end
